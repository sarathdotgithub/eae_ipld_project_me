...
temps_df["AvgTemperatureCelsius"] = (temps_df["AvgTemperatureFahrenheit"] - 32) * 5/9
       # uncomment this line to complete it
...
min_temp_city = temps_df[temps_df['AvgTemperatureCelsius'] == min_temp]['City'].iloc[0]
min_temp_date = temps_df[temps_df['AvgTemperatureCelsius'] == min_temp]['Date'].iloc[0]

max_temp_city = temps_df[temps_df['AvgTemperatureCelsius'] == max_temp]['City'].iloc[0]
max_temp_date = temps_df[temps_df['AvgTemperatureCelsius'] == max_temp]['Date'].iloc[0]
...
if unique_cities_list is not None and len(selected_cities) > 0:
    ...
    plt.title(f"Temperature in {selected_cities} from {start_date} to {end_date}")   # TODO
    ...
    for city in selected_cities:
        city_df = temps_df[temps_df['City'] == city]          # TODO: get a dataframe with the rows of the selected city
        city_df_period = city_df[(city_df['Date'] >= start_date) & (city_df['Date'] <= end_date)]   # TODO: get a dataframe with the rows of the selected city and the selected period of time using the Date column and any of the <, >, <=, >= operators to compare with start_date and end_date
        plt.hist(city_df_period['AvgTemperatureCelsius'], label=city, alpha=0.7)                # TODO plot each city histogram and use the label parameter to set the legend name for each city
